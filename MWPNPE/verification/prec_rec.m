%%%%%%%%%%%%
function [prec, tpr, fpr, thresh, AUC] = prec_rec(score, target, varargin)
% [~, ~,~ , ~, AUC] = prec_rec(cc(:,3), is_essential)
%  prec_rec(cc(:,3), is_essential);
%% huatu
% prec_rec(cc(:,3), is_essential);
% prec_rec(TEGS, IS_ESS,'holdFigure',1);
...
if (plot_pr || plot_roc)
    ...
    if (~hold_fig)
        ...
        if (plot_pr)
            ...
            plot([0; tpr], [1 ; prec], style,'Color','r','LineWidth',1.2); % add pseudo point to complete curve
        ...
        if (plot_roc)
            ...
            plot([0; fpr], [0; tpr], style,'Color','r','LineWidth',1.2); % add pseudo point to complete curve
        ...
        if (plot_pr)
            ...
            plot([0; tpr],[1 ; prec], style,'LineWidth',1.2); % add pseudo point to complete curve
        end
        ...
        if (plot_roc)
            ...
            plot([0; fpr], [0; tpr], style,'LineWidth',1.2);
        end
    end
end
AUC=trapz(fpr,tpr);
